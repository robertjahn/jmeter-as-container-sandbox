...
docker run --name jmeter-test -v "${PWD}/scripts":/scripts -v "${PWD}/$2":/results -v "${PWD}/results_raw":/results_raw --rm -d jobjahn/jmeter ./jmeter/bin/jmeter.sh -n -t /scripts/$1 -e -j /results_raw/jmeter.log -o /results -l /results_raw/result.tlf -JSERVER_URL="$3" -JDT_LTN="$DT_LTN" -JVUCount="$VUCount" -JLoopCount="$LoopCount"
